...
resolve_firebase_bin() {
  ...
  while IFS= read -r candidate; do
    ...
    # Avoid broken project-local shims inside checked-in node_modules.
    if [[ "$candidate" == "$ROOT_DIR/node_modules/.bin/firebase" ]] || [[ "$candidate" == *"/node_modules/.bin/firebase" ]]; then
      continue
    fi
    echo "$candidate"
    return 0
  done < <(which -a firebase 2>/dev/null | awk '!seen[$0]++')

  return 1
}

if ! FIREBASE_BIN="$(resolve_firebase_bin)"; then
  echo "firebase CLI is required. Install with: npm i -g firebase-tools"
  exit 1
fi

if ! "$FIREBASE_BIN" --version >/dev/null 2>&1; then
  echo "Firebase CLI binary is not usable: $FIREBASE_BIN"
...
echo "  firebase bin: $FIREBASE_BIN"
echo
