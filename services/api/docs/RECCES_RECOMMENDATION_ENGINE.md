...
- `functions/src/recces/firestoreReccesRepository.ts`
  ...
  - Resolves a single post by ID (`{author}:{essay}:{index}`) for feedback profile updates.

- `functions/src/recces/firestoreReccesUserProfileRepository.ts`
  - Stores per-user recommendation profile at:
    - `userRecommendationProfiles/{uid}`
  - Persists:
    - `themeWeights` (map of normalized theme key -> weight)
    - `signalCount`
    - `updatedAt`
  - Applies feedback deltas with decay and bounded weights so profiles stay stable over time.
...
- `functions/src/services/reccesRecommendationService.ts`
  ...
  - Scores candidates using:
    ...
    - persistent profile theme boost from `userRecommendationProfiles/{uid}`
    - liked-theme boost
...
```json
{
  ...
  "data": {
    ...
    "meta": {
      ...
      "profileSignalsUsed": 84,
      "profileThemesTracked": 19
    }
  }
}
```
...
- Score:
  ...
  - `+ profile_theme_boost` (positive for liked themes, negative for disliked themes)
  - `+ liked_theme_boost`
  - `+ small deterministic exploration noise`
- Diversity:
