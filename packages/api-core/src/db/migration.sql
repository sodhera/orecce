...
create index if not exists idx_analytics_events_occurred
  on analytics_events_raw (occurred_at desc);
...
create index if not exists idx_analytics_events_session_occurred
  on analytics_events_raw (session_id, occurred_at desc);
...
create or replace view analytics_daily_user_facts as
select
...
create or replace view analytics_daily_content_facts as
select
  date_trunc('day', occurred_at) as event_date,
  platform,
...
create or replace view analytics_funnel_facts as
select
  date_trunc('day', occurred_at) as event_date,
  platform,
...
create or replace view analytics_recommendation_outcomes as
select
  platform,
  properties->>'post_id' as post_id,
  min(properties->>'author_id') as author_id,
