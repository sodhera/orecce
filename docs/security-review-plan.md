...
Initial baseline from this first code sweep:

- Supabase bearer-token verification exists in the core API and Next.js API routes.
- Core request parsing already uses Zod for key request bodies and analytics batches.
- The core API currently allows any origin via `cors({ origin: true })`.
- Both the Express API and Next.js API routes use service-role Supabase clients on the server.
- Some web RLS and policy SQL currently lives under `apps/web/*.sql` instead of the core migration path.
- Abuse controls are inconsistent; curate chat has limits, but not every expensive route does.
- Mobile iOS currently allows arbitrary insecure loads in `apps/mobile/app.json`.
...
| Web (`apps/web`) | Browser session, direct Supabase client access, Next.js API routes, local storage, user feedback and curation flows | XSS, broken access control, RLS drift, CSRF assumptions, weak browser headers, over-broad client data access | CSP/header strategy, route authz review, RLS verification, input/output handling review, low-sensitivity client storage |
...
1. Web-owned policy SQL is split across ad hoc files under `apps/web/`, while the core migration path does not yet encode the same controls.
2. The core API uses permissive dynamic CORS instead of an explicit production origin allowlist.
3. Service-role Supabase clients are the default server-side data path, so route-level authorization bugs would bypass RLS as a safety net.
4. Expensive or write-heavy endpoints do not yet have a consistent rate-limiting and abuse-budget story.
5. Mobile iOS currently allows arbitrary insecure network loads and needs a production transport review.
6. Browser security headers and CSP are not yet codified in the web app configuration.
